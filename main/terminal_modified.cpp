...
std::vector <int> unknown_token_locations_within_token;
...
namespace types{
    ...
    std::vector <std::string> factual_type{"HOW", "WHAT", "WHO", "WHEN", "WHERE", "KAPP"}; // e407a35cb26b5f8cba43b38d9aaeb2d894da09135792ca375878751b0292ac0e636ce325d9462fa5e2e290ce4dd6542948ff977bb8298265cef4f02f386f5497
        ...
        // Debugging messages if prompted
            ...
            if (completion_flag==false && query != "exit"){
                ...
                if(self_learning_check(query_upper)==true){
                    ...
                    unknown_token_locations_within_token.clear();
                    unknown_tokens = fetch_unknown_tokens(token);
                    std::vector<std::string>::iterator it = token.begin();
                    while((it = std::find_if(it, token.end(), [](std::string x){return x == "?";})) != token.end()){

                        unknown_token_locations_within_token.push_back(std::distance(token.begin(), it));
                        it++;

                    }
                    for(int testcount = 0; testcount < unknown_token_locations_within_token.size(); testcount++){
                        std::cout << 1 << std::endl;
                        std::cout << unknown_token_locations_within_token[testcount] << std::endl;
                    }
                    ...
                    if(std::find()){
                        ...
                        // Request model responses, until a 
                        std::cout << "Input model responses - input [[]] token when finished." << std::endl;
